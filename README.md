...
DeVolt is a pioneering company in the electric vehicle (EV) charging ecosystem, simplifying the process of EV charging by tokenizing energy. This allows users to seamlessly buy energy from producers without dealing with blockchain complexity. The platform manages all transaction fees, making it easy and efficient for users while ensuring scalability using compressed accounts and tokens.

Leveraging the power of the **Solana blockchain**, our flagship product, the **DeVolt Payment Engine**, facilitates seamless energy transactions through the minting and burning of **VOLT tokens**. By integrating the **Light Protocol** for **Zero-Knowledge (ZK) Compression**, DeVolt ensures enhanced privacy and scalability, setting new standards in the decentralized energy marketplace.
...
- **API (`api`)**: Backend service built with Rust, interfacing with the blockchain and providing endpoints for frontend applications.
- **Payment Energy Service (`payment-energy-service`)**: Handles business logic for energy transactions, including buying, selling, and confirming transactions, with integration of ZK Compression for enhanced privacy.
- **Mobile Application (`mobile`)**: A user-facing application enabling EV owners and energy producers to interact with the DeVolt network, manage their accounts, and execute transactions.
...
- **Light Protocol CLI**: [Installation Guide](https://lightprotocol.io/docs/getting-started)
...
# Private Keys (Do NOT expose these in public repositories)
...
# Keypair Bytes (Ensure these are securely stored and not exposed)
...
Deploying program "payment_engine"...
Program path: /Users/username/Development/solana/devolt/devolt-radar/target/deploy/payment_engine.so...
Program Id: AWy7WdHbobYLzPFiKnkbeasjzu54okMxDWUgaJhbCffr
...
3. **Validate Solana Test Validator:**
...
**Common Error: Refunds Due to Insufficient Funds**

- **Cause:** Insufficient SOL or USDC balance in the relevant accounts.
- **Solution:** Use the provided `ensureSolBalance` and `ensureDevoltUsdcBalance` functions to guarantee sufficient funds before transactions.
...
- **Light Protocol Integration:** The integration of the Light Protocol for ZK Compression enhances the scalability and privacy of transactions. Ensure that the Light Protocol services are correctly configured and running alongside your Solana test validator.
- **Token Accounts Caching:** The application caches token account addresses to optimize performance. If you encounter issues with token accounts, consider clearing the cache or verifying the cached addresses.
- **Security:** Always keep your private keys secure. Never expose your private keys in public repositories or logs.
